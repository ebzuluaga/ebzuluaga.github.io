...
```sh
sudo blkid -s PARTUUID -o value /dev/sda2 # outputs the UUID of /dev/sda2
```
...
5. Install a display manager like `ly`
```sh
sudo pacman -S ly
# for ly, since it's a tty application we have to swap getty for ly
...
- Le minimalist computing lifestyle
	...
	- `zathura`: read pdfs with a minimalistic interface and keyboard controls
- "work"
	- Godot IDE
	- Krita
	- GIMP
